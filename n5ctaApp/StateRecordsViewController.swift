//
//  StateRecordsViewController.swift
//  n5ctaApp
//
//  Created by Maeve McCoy on 2/23/18.
...
class StateRecordsViewController: UIViewController, UIPickerViewDataSource, UIPickerViewDelegate, MFMailComposeViewControllerDelegate {
    
    @IBOutlet weak var chooseStateRecordsButton: UIButton!
    ...
    var seasonOptions: [String] = ["Outdoor", "Indoor"]
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if(currentCategory == "Relays") {
            let nextSceneRelayStateRecordsIndoor = segue.destination as? RelayStateRecordsTableViewController
            nextSceneRelayStateRecordsIndoor?.gender = currentGender
            nextSceneRelayStateRecordsIndoor?.category = currentCategory
            nextSceneRelayStateRecordsIndoor?.season = currentSeason
        } else {
            let nextSceneStateRecordsIndoor = segue.destination as? StateRecordsTableViewController
            nextSceneStateRecordsIndoor?.gender = currentGender
            nextSceneStateRecordsIndoor?.category = currentCategory
            nextSceneStateRecordsIndoor?.season = currentSeason
        }
    }
    
    // Choose season and click button to continue
    // TODO: add more segues as more API calls are finalized
    @IBAction func chooseStateRecords(_ sender: UIButton) {
        if(currentCategory == "Relays") {
            self.performSegue(withIdentifier: "relayStateRecordsSegue", sender: self)
        } else {
