...
class ViewController: UIViewController, UIPickerViewDataSource, UIPickerViewDelegate, MFMailComposeViewControllerDelegate {
    ...
    @IBOutlet weak var chooseAllTimeListButton: UIButton!
    ...
    func pickerView(_ pickerView: UIPickerView, didSelectRow row: Int, inComponent component: Int)
    {
        ...
        if (pickerView == seasonPicker) {
            ...
            if (currentSeason=="Cross Country") {
                ...
                self.eventCourseLabel.sizeThatFits(CGSize(width: 62, height: 20.5))
                ...
                self.eventCourseLabel.sizeThatFits(CGSize(width: 44.5, height: 20.5))
            }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "outdoorAllTimeSegue" ,
        ...
        } else if segue.identifier == "indoorAllTimeSegue",
            let nextSceneIndoor = segue.destination as? IndoorTableViewController {
            nextSceneIndoor.event = currentEvent
            nextSceneIndoor.gender = currentGender
            nextSceneIndoor.category = currentCategory
            nextSceneIndoor.season = currentSeason
    ...
    @IBAction func chooseAllTimeList(_ sender: UIButton) {
        if (currentSeason == "Outdoor") {
            self.performSegue(withIdentifier: "outdoorAllTimeSegue", sender: self)
        } else if (currentSeason == "Cross Country" ) {
            self.performSegue(withIdentifier: "xcAllTimeSegue", sender: self)
        } else if (currentSeason == "Indoor") {
            self.performSegue(withIdentifier: "indoorAllTimeSegue", sender: self)
        }
    }
